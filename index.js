...
const { botID } = require('./src/config/config.json');
const fs = require('fs'); const ee = require('events'); var events = new ee();
const http = require('http')
require('dotenv').config()
http.createServer((req, res) => res.end(`${client.user.username} have been deployed!`)).listen(3000)
console.clear()
require('./src/utils/mongoose')();
process.on('unhandledRejection', error => { console.error("\x1b[41m", `[UNHANDLED ALARM] ${error}`, "\x1b[0m"); console.log(error) }); client.on('shardError', error => { console.error(`[SHARD ALARM] ${error}`); console.log(error) });
client.commands = new Collection(); client.players = new Collection(); client.searchBy = new Collection();
...
loadSlash(process.env.token, botID); loadPlayers(client);
