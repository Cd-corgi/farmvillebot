...
module.exports = {
  ...
  async run(client, interaction) {
    ...
    if (!member) {
      ...
      let info = new Discord.EmbedBuilder().setTitle(`${interaction.user.username.endsWith("s") ? `${interaction.user.username}'` : `${interaction.user.username}'s`} Farmer tag`).addFields({ name: "🏠 House ID", value: `\`${consult.houseId}\``, inline: true, }, { name: "👛 Money", value: `💵 \`${consult.economy.money}\``, inline: true, }, { name: "💰 Bank", value: `💵 \`${consult.economy.bank}\``, inline: true, }).setThumbnail(interaction.user.displayAvatarURL()).setColor("Green")
      ...
      interaction.followUp({ embeds: [new EmbedBuilder().setTitle(`${member.username.endsWith('s') ? `${member.username}'` : `${member.username}'s`} Farmer tag`).addFields({ name: `🏠 House ID`, value: `${consult.houseId}`, inline: true }, { name: `👛 Money`, value: `💵 \`${consult.economy.money}\``, inline: true }, { name: `💰 Bank`, value: `💵 \`${consult.economy.bank}\``, inline: true }).setThumbnail(member.displayAvatarURL()).setColor("Green")] })
    }
  },
};
