...
const { generateCrateLoot, givingItems } = require('../../utils/functions')
        ...
        setTimeout(async () => {
            ...
            if (itemEffects.items[miscData].effects !== undefined) {
                ...
                if (getItemInfo.type == "crate") {
                    getItemInfo.amount--
                    if (getItemInfo.amount < 1) getUser.inventory = getUser.inventory.filter((x) => x.amount > 0)
                    let getList = await generateCrateLoot(getItemInfo)
                    getList.forEach(async (x, i) => {
                        // console.log(x)
                        await givingItems(x, getUser)
                    })
