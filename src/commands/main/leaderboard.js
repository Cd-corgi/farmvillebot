...
module.exports = {
    ...
    async run(client, interaction) {
        let getPlayers = await farmer.find({}).limit(50)
        ...
        for (let i = 0; i < definitiveList.length; i += limixemebd) { let users = definitiveList.slice(i, i + limixemebd); const embed = new EmbedBuilder().setThumbnail(client.user.displayAvatarURL()).setTitle(`📦 Top #50 Global Leaderboard`).setDescription(users.join('\n')).setColor("Red"); Embeds.push(embed) }
        ...
        if (Embeds.length < 2) { row.components[0].setDisabled(true); row.components[1].setDisabled(true); row.components[2].setDisabled(true); return interaction.reply({ embeds: [Embeds[0].setFooter({ text: `${totalPlayers.length} Players Ranked!` })], components: [row] }) }
        var curPage = 0
