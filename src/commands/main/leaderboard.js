...
module.exports = {
    ...
    async run(client, interaction) {
        ...
        for (let i = 0; i < totalPlayers.length; i++) { let getUser = await client.users.fetch(totalPlayers[i].userId); definitiveList.push(`${i + 1 == 1 ? `🥇 \`1\`` : i + 1 == 2 ? `🥈 \`2\`` : i + 1 == 3 ? `🥉 \`3\`` : `\`🔹 ${i + 1}\``} **${getUser.username}** 💵 \`${totalPlayers[i].totalMoney}\``) }
        ...
        const filter = (i) => { if (i.user.id !== interaction.user.id) { return interaction.reply({ content: `⚠ \`This interaction is not executed by you!\``, ephemeral: true }) } }
        const collector = interaction.channel.createMessageComponentCollector({ time: 60000, idle: 1000, filter, componentType: Discord.ComponentType.Button })
            ...
            switch (i.customId) {
                case "redo":
                    if (curPage !== 0) {
                        ...
                        if (curPage == 0) { row.components[0].setDisabled(true) } else { row.components[0].setDisabled(false) }
                ...
                case "forw":
                    if (curPage < Embeds.length - 1) {
                        ...
                        if (curPage == Embeds.legnth - 1) { row.components[2].setDisabled(true) } else { row.components[2].setDisabled(false) }
                        i.editReply({ embeds: [Embeds[curPage].setFooter({ text: `${totalPlayers.length} Players Ranked!` })], components: [row] })
                    }
                    collector.resetTimer()
                    break;
        ...
        collector.on("end", async (_, reason) => {
            if (reason == "user" || reason == "time" || reason == "idle") {
                try {
                    row.components[0].setDisabled(true); row.components[1].setDisabled(true); row.components[2].setDisabled(true)
