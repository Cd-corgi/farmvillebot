...
module.exports = {
        ...
        collection.on("collect", async i => {
            ...
            switch (i.componentType) {
                ...
                case 3:
                    ...
                    switch (i.values[0]) {
                        case "seed":
                            getUpdateItems.inventory = getUpdateItems.inventory.filter((v) => v.type == "seed")
                            ...
                            break;
                        case "tool":
                            getUpdateItems.inventory = getUpdateItems.inventory.filter((v) => v.type == "tool")
                            if (getUpdateItems.inventory.length < 1) {
                                i.editReply({ embeds: [new Discord.EmbedBuilder().setColor("Red").setDescription(`❌ There's no item with this type! Clearing Filter...`)], components: [], ephemeral: true })
                                setTimeout(async () => {
                                    Embeds = await clearFilter(client, i, Embeds);
                            ...
                            break;
                        case "products":
                            getUpdateItems.inventory = getUpdateItems.inventory.filter((v) => v.type == "products")
                            if (getUpdateItems.inventory.length < 1) {
                                i.editReply({ embeds: [new Discord.EmbedBuilder().setColor("Red").setDescription(`❌ There's no item with this type! Clearing Filter...`)], components: [], ephemeral: true })
                                setTimeout(async () => {
                                    Embeds = await clearFilter(client, i, Embeds);
                            ...
                            break;
                        case "misc":
                            getUpdateItems.inventory = getUpdateItems.inventory.filter((v) => v.type == "misc")
                            if (getUpdateItems.inventory.length < 1) {
                                i.editReply({ embeds: [new Discord.EmbedBuilder().setColor("Red").setDescription(`❌ There's no item with this type! Clearing Filter...`)], components: [], ephemeral: true })
                                setTimeout(async () => {
                                    Embeds = await clearFilter(client, i, Embeds);
                            ...
                            break;
                        case "token":
                            getUpdateItems.inventory = getUpdateItems.inventory.filter((v) => v.type == "token")
                            if (getUpdateItems.inventory.length < 1) {
                                i.editReply({ embeds: [new Discord.EmbedBuilder().setColor("Red").setDescription(`❌ There's no item with this type! Clearing Filter...`)], components: [], ephemeral: true })
                                setTimeout(async () => {
                                    Embeds = await clearFilter(client, i, Embeds);
                            ...
                            break;
                        case "crate":
                            getUpdateItems.inventory = getUpdateItems.inventory.filter((v) => v.type == "crate")
                            if (getUpdateItems.inventory.length < 1) {
                                i.editReply({ embeds: [new Discord.EmbedBuilder().setColor("Red").setDescription(`❌ There's no item with this type! Clearing Filter...`)], components: [], ephemeral: true })
                                setTimeout(async () => {
                                    Embeds = await clearFilter(client, i, Embeds);
    ...
    async givingItems(gg, user) {
        ...
        if (gg.item == "Money") {
            ...
            var getItemPos = user.inventory[user.inventory.map((v) => v.name).indexOf(gg.item)]
            getItemPos.amount++
        } else {
    ...
    async generateCrateLoot(item) {
        ...
        if (item.name === "Old Box") {
            for (let i = 0; i < 2; i++) {
                ...
                itemList.push(getI)
                // crate.OldBox = crate.OldBox.filter((v) => v.name != getI.name)
            }
            return itemList
        }
        if (item.name === "Wooden Crate") {
            for (let i = 0; i < 4; i++) {
                ...
                itemList.push(getI)
                // crate.WoodenCrate = crate.WoodenCrate.filter((v) => v.name != getI.name)
            }
            return itemList
        }
        if (item.name === "Old Shipment Crate") {
            for (let i = 0; i < 6; i++) {
                ...
                itemList.push(getI)
                // crate.OldShipmentCrate = crate.OldShipmentCrate.filter((v) => v.name != getI.name)
            }
            return itemList
        }
    }
