...
module.exports = async (client, interaction) => {
    ...
    if (interaction.type == Discord.InteractionType.ApplicationCommandAutocomplete) {
        ...
        switch (cmd.data.name) {
            ...
            //     if (!consult) return;
            //     if (consult.inventory.length < 1) return;
            //     consult.inventory = consult.inventory.filter((x) => x.type == "products" && x.amount >= 1 || x.type == "tool" && x.amount >= 1 || x.type == "misc" && x.amount >= 1)
            //     var choices = []
            //     consult.inventory.forEach((v) => { choices.push(`${v.emoji} ${v.name}`) })
            //     try {
